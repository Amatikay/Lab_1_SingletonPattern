...
public:
    ...
    //Для решения этой проблемы можно в качестве аргумента передавать поток, и пусть метод из него получает данные
    //readPersons(std::istream& input)  и writePersons(std::ostream& output)
    //Я этого не делаю, так как в задании четко сказано получать данные из файла и выводить в файл.
